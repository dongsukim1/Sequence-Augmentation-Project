...
def seq_collator(data_dict_list, tokenizer, max_length=500, add_special_tokens=False):
    sample = collate_dict(data_dict_list)
    inputs = tokenizer.batch_encode_plus(sample["seq"],
                                         add_special_tokens=add_special_tokens,
                                         padding='max_length',
...
def get_custom_seq_collator(tokenizer, max_length=500, add_special_tokens=False):
    return functools.partial(seq_collator, tokenizer=tokenizer, max_length=max_length, add_special_tokens=add_special_tokens)

def seq_collator_vocab(lst, vocab, device=None):
    sequences, labels = zip(*lst)
    return vocab.to_input_tensor(sequences, device=device), torch.stack(labels)

def get_seq_collator(vocab, device=None):
    return functools.partial(seq_collator_vocab, vocab=vocab, device=device)
